...
class MAIUINaivigationAgent(MCPAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Handle resize parameters from CLI kwargs
        cli_resize_image = kwargs.get("resize_image", None)
        cli_resize_resolution = kwargs.get("resize_resolution", None)
        ...
        default_conf = {
            ...
            "max_tokens": 2048,
            "resize_image": False,
            "resized_image_width": 720,
            "resized_image_height": 1600,
        }
        self.runtime_conf = {**default_conf, **runtime_conf}

        # Override with CLI parameters if provided
        if cli_resize_image is not None:
            self.runtime_conf["resize_image"] = cli_resize_image
        if cli_resize_resolution is not None:
            try:
                w, h = cli_resize_resolution.split("x")
                self.runtime_conf["resized_image_width"] = int(w)
                self.runtime_conf["resized_image_height"] = int(h)
            except (ValueError, AttributeError):
                logger.warning(f"Invalid resize_resolution format: {cli_resize_resolution}, expected WIDTHxHEIGHT, using default 720x1600")
    ...
    def predict(self, observation: dict[str, Any]) -> tuple[str, JSONAction]:
        ...
        # Resize for model input, keep original for coordinate mapping
        if self.resize_image:
            display_image = obs_image.resize((self.resized_width, self.resized_height))
        else:
            display_image = obs_image

        self.history_images.append((display_image, tool_call, ask_user_response))
