                ...
                if match_path:
                    ...
                    if match_added:
                        ...
                        if date_added > int(timestamp):
                            ...
                            logger.info(
                                f"  - Confirmed new photo: {os.path.basename(path)} (added at {date_added})"
                            )
                        else:
                            logger.info(
                                f"  - Filtered out old photo: {os.path.basename(path)} (added at {date_added}, not after {int(timestamp)})"
                            )
                    else:
...
class TakeSelfieAskUserTask2(BaseTask):
    task_tags = {"agent-user-interaction", "lang-en"}

    goal = "Take a photo, and share it with Henry via email."

    correct_email = "henry.wright@harbortech.com"
    ...
    def initialize_task_hook(self, controller: AndroidController) -> bool:
        ...
        self.relevant_information = (
            "Hery's email can be found in the Contacts app. His full name is Hery Wright."
        )
    ...
    def is_successful(self, controller: AndroidController) -> tuple[float, str]:
        ...
        logger.info("-" * 70)
        logger.info(f"RESULT: Found {len(new_photo_names)} new photo(s)")
        if new_photo_names:
            logger.info(f"New photo names: {new_photo_names}")
        logger.info("=" * 70)
        ...
        if email_sent:
            # Check if attachment matches any new photo
            match_found = False
            matched_attachment = None
            matched_new_photo = None
            ...
            if attachment_filenames and new_photo_names:
                for attachment_name in attachment_filenames:
                    ...
                    for new_photo_name in new_photo_names:
                        ...
                        # Method 1: Exact filename match (case insensitive)
                        if attachment_name.lower() == new_photo_name.lower():
                            match_found = True
                            matched_attachment = attachment_name
                            matched_new_photo = new_photo_name
                            break

                        # Method 2: Base name match (ignoring extension)
                        ...
                        # Method 3: Fuzzy match - one contains the other
                ...
                if not match_found:
                    for attachment_name in attachment_filenames:
                        ...
                        if attachment_base in new_image_ids:
                            match_found = True
                            matched_attachment = attachment_name
                            matched_new_photo = f"ID:{attachment_base}"
