...
class CheckCandidateAskUserTask2(BaseTask):
    task_tags = {"agent-user-interaction", "lang-en"}

    goal = (
        "Find Kevin's resume and check whether his GPA matches my job requirements. "
        "If yes, send a text message to HR: 'please pass the application for Kevin.' "
        "If no, send a text message to HR: 'please reject the application for Kevin.'"
    )
    hr_name = "Mia Scott"
    hr_phone_number = "14058298746"
    expected_message_partial = "please pass the application for Kevin"
    job_requirements = "The GPA of the job candidate should be higher than 3.6."
